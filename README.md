...
This script clones the upstream repo fresh, applies idempotent patches for all of the above, installs dependencies with a multi-tier fallback strategy (including Termux User Repository / TUR precompiled PyPI wheels), and then runs two rounds of live self-tests to confirm the server actually works — not just that it imports.

## What it does (high level)

1. **Phase 0 — Storage pre-flight:** warns early if free space on `$HOME` is low, before the heavy native builds start.
2. **Phase 1 — Termux environment:** updates `pkg`, non-interactively installs required native toolchain packages (`rust`, `clang`, `libxml2`, `libxslt`, `binutils`, etc.) in optimized batches, and upgrades `pip`.
...
4. **Phase 3 — Launcher & client config:** writes a `run.sh` launcher (with automatic `termux-wake-lock` integration) and an MCP client config JSON snippet with concrete absolute paths you can merge into Claude Desktop / Cursor / etc.
...
- `~/.mcpsearch/run.sh` — launcher script (with absolute interpreter, app paths, and wake-lock management)
...
├── install_mcpsearch.sh   # The core installer — clone, patch, install, self-test (v1.9.2)
